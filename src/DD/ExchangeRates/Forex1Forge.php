...
class Forex1Forge {
	 ...
	 */
	public function GetExchangeRates(string $currencyPairs, string $delimiter = ",") : array {

		$currencyPairs1 = strtoupper ($currencyPairs);
		$arrayPairs     = explode($delimiter, $currencyPairs1);

		foreach($arrayPairs AS $pair){

			if(!in_array ($pair,$this->possibleRates)) {
				throw new ValidationException("Pair: ".$pair." is not available");
			}

		}

		$currencyPairs1 = str_replace($delimiter,self::DELIMITER, $currencyPairs1);

		$curl = curl_init (self::URL."/".self::VERSION."/quotes?pairs=".$currencyPairs1."&api_key=".$this->apiKey);
		...
		$fieldName = $http_code == 200 ? 'result' : 'error';
		$this->responseArray[$fieldName] = json_decode ($response,true);
